...
egyezo_sorok_szama <- napi_szurt_rendall %>%
  ...
  nrow()
egyezo_sorok_szama
#itt vannak bőven olyan sorok, amik megegyeznek. ez akkor baj, ha a teljes sor
#ismétlődik, azaz ugyanarra az év+hónap+nap+irány kombináció ismétlődik. Ha esetleg
#ezeknél a Nyers érték eltér, azzal lehet valamit kezdeni, de azt majd csak ezután
#nézzük meg

egyezo_sorok_napi_szurt_rendall <- napi_szurt_rendall %>%
  count(azonosito, ev, honap, nap, irany) %>%
  filter(n > 1)
...
#többi nap végig 0, így a szezon átlag 0.01, a relatívja pedig 182 lett.
#érdekes még, hogy elfelejtettem az őszre szűrni, mégis csak 2022 őszi megfigyelé-
#sek vannak, illetve az itteni sorok száma is 182 (két irány, 91 nap).

nyirtelek_df <- napi_adatok_norm_szurt %>%
  filter(azonosito == "115004")

unique(nyirtelek_df$ev)
unique(nyirtelek_df$evszak)

#a napi_adatok df-ben még volt 2023 is és mind a 4 évszakra megfigyelés, ezek a 
#szűrésben lettek eltávolítva (0.25 feletti rendállások megtartása)
#a renelkezésre állásnak (akár a sima, akár a summázottnak) ugyanakkor sehogy sem
#szabadna 0.98-nak lennie. Ha a napokat nézzük, akkor 91 napon volt 1 db mérése 
#(az is csak az egyik irányban), ami ~1%-os rendállás.

#ezek az outlierek akkor is bent maradnak, ha a df_rend_allas_szurt-et használom,
#hiszen az ottani 0.25-ös érték 3-hoz van viszonítva, azaz 0.25/3 ~ 8% feletti
#rendállásokat tartja meg.

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
######## 14.3. MODELLEK ILLESZTÉSE A D-STATRA A RENDÁLLÁS FÜGGVÉNYÉBAN  #########
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #

#érdemes lehet statisztikai bizonyítékot is találni arra, hogy a rendelkezésre
#állás adataiból következtethetünk a statisztikai értékre, vagyis arra, hogy
#a állomás adatainak eloszlása megfelelő-e, kb.  adatminőség vs statisztikai megbíz-
#hatóság lesz itt tesztelve.
#Mivel a plotok alapján nem jelenthető ki egyértelműen, hogy a két változónk kapcso-
#lata lineáris, ezért az egyszerű lin modell mellett a nemlineáris kapcsolatokat
#modellezésére használt GAM-modellt is lefuttatom. Ennél a edf értéket érdemes
#elsőre nézni, ami ha ~1, akkor nincs szükség a modellre, mert a kapcsolat lineáris-
#nak tekinthető, és minél nagyobb 1-nél, annál inkább indokolt a használata.
#Illetve kértem egy ANOVA táblát is, ami ha szignifikáns, akkor a GAM modell jobban
#illeszkedik az adatokra, mint a linmodell. Az ökölszabály azonban az edf-ből való
#kiindulás (ezt egyébként a summary "Approximate significance of smooth terms"
#részében találjátok)

#függvény definiálása
illeszkedes_ellenorzes <- function(allomas_szuro_df,
                                   rendallas_df,
                                   target_azonosito) {
  
  # 1. Szűrés: csak azok az esetek, ahol a target állomás szerepel
  szurt_df <- allomas_szuro_df %>%
    dplyr::filter(allomas_A == target_azonosito | allomas_B == target_azonosito)
  
  # 2. "Csere" logika: a target állomás legyen mindig az A oldalon
  rendezett_df <- szurt_df %>%
    dplyr::mutate(
      csere = allomas_B == target_azonosito,
    ...
    dplyr::transmute(
      ...
      allomas_A_elem = uj_allomas_A_elem,
      allomas_B_elem = uj_allomas_B_elem,
      dplyr::across(c(evszak, statistic, p_value, p_adjusted_BH, `Szignifikáns-e az eltérés?`))
    )
  
  # 3. Join a másik állomás rendelkezésre állásával
  rendezett_df <- rendezett_df %>%
    dplyr::left_join(
      rendallas_df %>% 
        dplyr::select(azonosito, evszak, sum_rend_allas),
  ...
  # Ellenőrzés: van elég adat?
  if (nrow(korr_df) < 5) {
    warning("Túl kevés adat áll rendelkezésre modellezéshez.")
  }
  
  # 5. GAM modell illesztése
  gam_mod <- mgcv::gam(statistic ~ s(sum_rend_allas), data = korr_df)
  
  # 6. Lineáris modell illesztése
  lm_mod <- lm(statistic ~ sum_rend_allas, data = korr_df)
  
  # 7. Kimenet: summary-k és anova-k listában
  return(list(
    gam_mod = gam_mod,
    lm_mod = lm_mod,
...
#TAVASZRA - 119004 Zánka

modellek_tavasz <- illeszkedes_ellenorzes(
  allomas_szuro_df = ks_tavasz_teljes_rel,
  rendallas_df = df_rend_allas_agg,
  target_azonosito = "119004"
)

modellek_tavasz$gam_summary
modellek_tavasz$gm_summary
modellek_tavasz$anova
modellek_tavasz$aic_gam
modellek_tavasz$aic_lm

#minden ugyan az gyakorlatilag, nem kell a GAM-modell, ebben az esetben elég az
#egyszerű lineáris modell is

#NYÁRRA - 119006 Örvényes

modellek_nyar <- illeszkedes_ellenorzes(
  allomas_szuro_df = ks_nyár_teljes_rel,
  rendallas_df = df_rend_allas_agg,
  target_azonosito = "119006"
)

modellek_nyar$gam_summary
modellek_nyar$lm_summary
modellek_nyar$anova
modellek_nyar$aic_gam
modellek_nyar$aic_lm

#az AIC alacsonyabb a lin modellnél, ezt erősíti meg az ANOVA is, viszont az edf
#értéke 2 fölötti, ami alapján a kapcsolat a d-stat és a rendállás között nem li-
#neáris. Nézzük meg:

ggplot(modellek_nyar$valtozok_df, aes(sum_rend_allas, statistic)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "gam", formula = y ~ s(x)) +
  theme_minimal()

#nagyon nagy az adatok szórása, így lehet, hogy a lm jobban becsül, de az a modell
#sem lett szignifikáns (sőt, az adj.R^2 negatív, tehát ha csak a D-stat átlagával
#becsülnék is pontosabb lennék, mint a lin modellel. 
#Tanulság ebből talán az, hogy ebben az esetben nincsen kimutatható kapcsolat a
#változók között.

#ŐSZRE - 119003 Balatonederics

modellek_osz <- illeszkedes_ellenorzes(
  allomas_szuro_df = ks_ősz_teljes_rel,
  rendallas_df = df_rend_allas_agg,
  target_azonosito = "119003"
)

modellek_osz$gam_summary
modellek_osz$lm_summary
modellek_osz$anova
modellek_osz$aic_gam
modellek_osz$aic_lm

#A GAM modellnél szignifikáns lett az együttható, az edf 1.89, az megmagyarázott
#deviancia 26.2%, azaz a teljes bizonytalanság kb negyedét sikerült ezzel a modellel
#megmagyarázni.
#A lin modell szintén szignifikáns lett, és mind az AIC mind az ANOVA alapján ez
#a jobb ebben az esetben.

osszehasonlito_plot_osz_rendall

#TÉLRE - 105003 Tarcal

modellek_tel <- illeszkedes_ellenorzes(
  allomas_szuro_df = ks_tél_teljes_rel,
  rendallas_df = df_rend_allas_agg,
  target_azonosito = "119003"
)

modellek_tel$gam_summary
modellek_tel$lm_summary
modellek_tel$anova
modellek_tel$aic_gam
modellek_tel$aic_lm

#Itt is pontosan ugyanazokat az értékeket kapjuk, és noha az ANOVA alapján a GAM
#jobban teljesít, az edf-je 1, tehát szerinte ez egy lineáris modell.

osszehasonlito_plot_tel_rendall #loess illesztéssel, hogy látni lehessen a trendet

#Konklúzió:
#sem a lineáris, sem a GAM modell nem vitt közelebb a megoldáshoz, statisztikai
#bizonyítékot nem tudtunk nyújtani arra a feltételezésre, hogy a rendelkezésre 
#állási adatok javulásával az adott mérőállomás eloszlása konvergál a referencia
#állomás eloszlásához.

####################### D stat helyett az ÁTLAGOS ELTÉRÉS #######################

#Nézzük meg ugyanezt csak nem a KS stat értékét használva, hanem az eloszlásfüggvények
#közti átlagos különbséget (ugye a D-stat a legnagyobb különbséget mutatja, függetle-
#nül az eloszlások globális képétől)

#a különbségek kiszámolása minden évszakra + a rendelkezésre állási adaotok csatolása

#tavaszra

ecdf_diff_osszes_tavasz <- ecdf_allomasok_evszak(napi_adatok = napi_adatok_norm_szurt,
                                                target_evszak = "tavasz",
                                                valtozo = "Relatív")

ecdf_diff_osszes_tavasz <- ecdf_diff_osszes_tavasz %>%
  left_join(
    df_rend_allas_agg_szurt %>%
      dplyr::select(azonosito, evszak, sum_rend_allas) %>%
      rename(allomas_A = azonosito,
             sum_rend_allas_A = sum_rend_allas),
    by = c("allomas_A", "evszak")
...
ecdf_diff_osszes_nyar <- ecdf_allomasok_evszak(napi_adatok = napi_adatok_norm_szurt,
                                                 target_evszak = "nyár",
                                                 valtozo = "Relatív")

ecdf_diff_osszes_nyar <- ecdf_diff_osszes_nyar %>%
  left_join(
    df_rend_allas_agg_szurt %>%
      dplyr::select(azonosito, evszak, sum_rend_allas) %>%
      rename(allomas_A = azonosito,
...
ecdf_diff_osszes_osz <- ecdf_allomasok_evszak(napi_adatok = napi_adatok_norm_szurt,
                                                 target_evszak = "ősz",
                                                 valtozo = "Relatív")

ecdf_diff_osszes_osz <- ecdf_diff_osszes_osz %>%
  left_join(
    df_rend_allas_agg_szurt %>%
      dplyr::select(azonosito, evszak, sum_rend_allas) %>%
      rename(allomas_A = azonosito,
...
ecdf_diff_osszes_tel <- ecdf_allomasok_evszak(napi_adatok = napi_adatok_norm_szurt,
                                                 target_evszak = "tél",
                                                 valtozo = "Relatív")

ecdf_diff_osszes_tel <- ecdf_diff_osszes_tel %>%
  left_join(
    df_rend_allas_agg_szurt %>%
      dplyr::select(azonosito, evszak, sum_rend_allas) %>%
      rename(allomas_A = azonosito,
...
#ez ugyanazt számolja és kb ugyanúgy működik, mint az előző, csak itt a GAM gauss
#helyett beta eloszlást használ, mert az avg_diff egy 0-1 közötti, százalékos el-
#térést tartalmazó változó, amire ezt ajánlják.
#Ez is ad egy edf értéket (itt kiírja Estimated degrees of freedom-ként), ezt kell
#első körben nézni (a total az interceptre meg az együtthatóra vonatkozik, annak itt
#mindig 2-nek kell lennie), illetve a Beta regression melleti zárójeles értéket az ún.
#precíziós érték, ami a fordítottan arányos a varral, azaz a minél nagyobb érték
#az adatok annál közelebb vannak a becsült görbéhez/egyeneshez.
#(A GAM summary-jében lesz egy REML score is, amivel nekünk most nem kell foglalkoznunk,
#mert ez csak más, ugyanezeken az adatokon futtatott modellekkel való összehasonlításra
#alkalmas)

#Az eredmények elemzését már nem csináltam meg!!!! 


illeszkedes_ellenorzes_avgdiff <- function(df, target_azonosito) {
  ...
  rendezett_df <- szurt_df %>%
    dplyr::mutate(
      ...
      uj_sum_rend_allas_A = dplyr::if_else(csere, sum_rend_allas_B, sum_rend_allas_A),
      uj_sum_rend_allas_B = dplyr::if_else(csere, sum_rend_allas_A, sum_rend_allas_B)
    ) %>%
    dplyr::transmute(
      allomas_A      = uj_allomas_A,
      allomas_B      = uj_allomas_B,
      ...
      allomas_A_elem = uj_allomas_A_elem,
      allomas_B_elem = uj_allomas_B_elem,
      sum_rend_allas_A = uj_sum_rend_allas_A,
      sum_rend_allas_B = uj_sum_rend_allas_B,
      evszak,
      avg_diff
    )
  
  # Ellenőrzés:
  if (nrow(rendezett_df) < 5) {
    warning("Túl kevés adat áll rendelkezésre modellezéshez.")
  }
  
  # 3. GAM modell illesztése 
  gam_mod <- mgcv::gam(
    avg_diff ~ s(sum_rend_allas_B),
    data = rendezett_df,
    family = mgcv::betar()
  )
  
  # 4. Lineáris modell
  lm_mod <- lm(avg_diff ~ sum_rend_allas_B, data = rendezett_df)
  ...
  return(list(
    ...
    aic_lm = AIC(lm_mod),
    anova = anova(lm_mod, gam_mod),
    valtozok_df = rendezett_df #hogy ellenőrizhető legyen
  ))
}

#függvény a GAM modell normális ábrázolására:

avg_diff_gam_plot <- function(illeszkedes_obj, title = NULL) {
  
  # Adatok és modell kinyerése
  df <- illeszkedes_obj$valtozok_df
  gam_mod <- illeszkedes_obj$gam_mod
  
  # Előrejelzési grid
  pred_df <- data.frame(
    sum_rend_allas_B = seq(
      min(df$sum_rend_allas_B, na.rm = TRUE),
      max(df$sum_rend_allas_B, na.rm = TRUE),
      length.out = 200
    )
  )
  
  # Predikció LINK skálán (betar!)
  preds_link <- predict(
    gam_mod,
    newdata = pred_df,
    se.fit = TRUE,
  ...
  # Visszatranszformálás RESPONSE skálára
  linkinv <- gam_mod$family$linkinv
  
  pred_df$fit   <- linkinv(preds_link$fit)
  pred_df$lower <- linkinv(preds_link$fit - 1.96 * preds_link$se.fit)
  pred_df$upper <- linkinv(preds_link$fit + 1.96 * preds_link$se.fit)
  
  # Plot
  p <- ggplot(df, aes(x = sum_rend_allas_B, y = avg_diff)) +
    
    geom_point(size = 2, alpha = 0.7) +
    
    geom_ribbon(
      data = pred_df,
      aes(x = sum_rend_allas_B, ymin = lower, ymax = upper),
      alpha = 0.2,
      inherit.aes = FALSE
    ) +
    ...
    labs(
      x = "Rendelkezésre állás (sum_rend_allas_B)",
      y = "ECDF átlagos eltérés (avg_diff)",
      title = ifelse(
        is.null(title),
        "GAM illesztés (beta) – avg_diff ~ s(sum_rend_allas_B)",
        title
      )
    ) +
...
modellek_avgdiff_tavasz <- illeszkedes_ellenorzes_avgdiff(df = ecdf_diff_osszes_tavasz,
                                                          target_azonosito = "119004")

avg_diff_gam_plot(modellek_avgdiff_tavasz, title = "Az átlagos eltérés és a rendelkezésre állás kapcsolata referencia állomás tükrében")

#NYÁRRA - 119006 Örvényes

modellek_avgdiff_nyar <- illeszkedes_ellenorzes_avgdiff(df = ecdf_diff_osszes_nyar,
                                                          target_azonosito = "119006")

avg_diff_gam_plot(modellek_avgdiff_nyar, title = "Az átlagos eltérés és a rendelkezésre állás kapcsolata referencia állomás tükrében")

#ŐSZRE - 119003 Bederics

modellek_avgdiff_osz <- illeszkedes_ellenorzes_avgdiff(df = ecdf_diff_osszes_osz,
                                                        target_azonosito = "119003")

avg_diff_gam_plot(modellek_avgdiff_osz, title = "Az átlagos eltérés és a rendelkezésre állás kapcsolata referencia állomás tükrében")

#TÉLRE - 105003 Tarcal

modellek_avgdiff_tel <- illeszkedes_ellenorzes_avgdiff(df = ecdf_diff_osszes_tel,
                                                        target_azonosito = "105003")

avg_diff_gam_plot(modellek_avgdiff_tel, title = "Az átlagos eltérés és a rendelkezésre állás kapcsolata referencia állomás tükrében")


# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
######## 14.4. TESZTSTATISZTIKA ÉS RENDÁLLÁSOK KAPCSOLATÁNAK VIZSGÁLATA #########
########                         SZUPERELOSZLÁS                         #########             
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #

#EZT MÉG NAGYON CSISZOLNI KÉNE!!!
